...
test("кнопка выхода не зависит от файла с корня сайта", async () => {
    // Вся её внешность приходит из /game-menu.css, которого в этом
    // репозитории нет. Не доехал — ссылка остаётся строчной, без места на
    // экране и без высоты, и требование «кнопка выхода есть всегда» рушится
    // молча. Здесь продублировано то, без чего ею нельзя пользоваться.
    const own = css.slice(css.indexOf('.game-home-menu {'));
    const rule = own.slice(0, own.indexOf('}'));
    for (const mark of [/position:\s*fixed/, /min-height:\s*44px/, /z-index/]) {
        assert.match(rule, mark, "подстраховка кнопки выхода неполная");
    }
});
