...
## Пульт сцены для витрины

- `stihii.scene()` ставит чистую арену и замирает; звук выключен.
- `stihii.cast('water')` кладёт воду в следующий слот (есть ещё `fire` и `wind`).
- `stihii.strike()` разыгрывает один красивый обмен с коронкой противника.
- `stihii.freeze()` останавливает текущий кадр для скриншота; следующий дубль начни с `scene()`.
- Снимая петлю, не жди результат `strike()`: запускай запись сразу после команды.

Это ручной пульт — он про нажатия. Рядом стоит пульт времени: он нужен, когда
петлю снимают покадрово и нельзя полагаться на часы браузера.

- `stihii.hold()` забирает время себе: таймеры боя и все анимации страницы
  начинают идти только от пульта. Без этого шага `step()` падает — иначе время
  течёт дважды, и мир проживает больше, чем насчитал снимающий.
- `stihii.step(секунды)` двигает мир и возвращает отпечаток нарисованного.
- `stihii.render()` заставляет браузер применить накопленные стили сейчас.
- `stihii.release()` возвращает настоящее время.
- `stihii.loop()` — готовая петля «противник бьёт коронкой, игрок её гасит».
  Падает, если снимать нечего. Шагать надо `while (!план.доиграно())`, но не
  дольше `план.кадровСзапасом`, а после — вызвать `план.проверить()`: он падает,
  если петля показала не то.

Длительность обмена константой не обещается: `CLASH_MS` не включает ожидание
анимаций внутри обмена. Замерено шагами — два обмена это 2367 мс, а не 2300.
